	...
	<div class="um-form" data-mode="<?php echo esc_attr( $mode ) ?>">
		...
		if ( um_is_on_edit_profile() || UM()->user()->preview ) {
			...
			if ( $menu_enabled || ! empty( $tabs[ $nav ]['hidden'] ) ) { ?>
				<style>
				.um-custom-profile-body.um-profile-body.main.main-default {
    				max-width: unset;
				}
				</style>
				<div class="um-custom-profile-body um-profile-body <?php echo esc_attr( $nav . ' ' . $nav . '-' . $subnav ); ?>">
					 ...
					 * ?>
					 */
					if( isset($_GET['profiletab']) && $_GET['profiletab'] !='main' ) {
						do_action("um_profile_content_{$nav}", $args);
					} else {
...
global $post, $wpdb;
$user_ultimate_profile_layout = esc_attr( get_the_author_meta( 'ultimate_profile_layout', $userID ) );
$default_ultimate_profile_layout = get_option( 'ultimate_profile_layout' );

if( $user_ultimate_profile_layout !=''  ){
	$layoutID = $user_ultimate_profile_layout;
} else {
	$layoutID = $default_ultimate_profile_layout;
}

$sql = "SELECT * FROM ".$wpdb->prefix."posts WHERE ID = '$layoutID' AND post_status = 'publish'";
